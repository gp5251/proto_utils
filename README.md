...
在 `.proto` 文件中,每个一元或服务端流的 rpc 方法上方都会出现「▶ 调用」CodeLens。点击后底部面板的 RPC 工作台自动滑出并预选该方法;光标停在某个 rpc 方法行上时,工作台进入**聚焦模式**——只显示当前接口的表单与结果,顶部「‹ 返回列表」可回到完整服务目录。也可以通过命令面板运行 **Proto Utils: Open RPC Runner**,或右键 `.proto` 编辑器选择同名命令,手动浏览服务和方法。
